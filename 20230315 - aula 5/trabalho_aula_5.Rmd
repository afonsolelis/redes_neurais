...
# Treinamento com kFold == 5 utilizando o código fornecido em sala de aula

```{r}
# Treinamento com método split
...
set.seed(123) # definindo uma semente para reproduzir os mesmos resultados
indices_treino <- sample(1:nrow(iris), round(0.7*nrow(iris)), replace = FALSE)
dados_treino <- iris[indices_treino, ]
dados_teste <- iris[-indices_treino, ]

# Preparando os dados de treino e teste
X_treino <- dados_treino[, -5]
X_teste <- dados_teste[, -5]
classes_bin_treino <- decodeClassLabels(dados_treino$Species)
classes_bin_teste <- decodeClassLabels(dados_teste$Species)
...
acuracia <- calc_acuracia(dados_teste$Species, y_estimado)

# Imprimindo a acurácia
print(acuracia)

# Plotando o gráfico de erro
plotIterativeError(model, main = "Erro Iterativo")
```

## Separação dos dados em 5 pastas para cross validation
```{r}
# Carregando bibliotecas
library(cvTools)
library(RSNNS)
...
--------------------------------------------------------------------------------
# Testes com outras bibliotecas e abordagens
## Treinamento com kFold == 5 utilizando a biblioteca nnet
...
## Treinamento com kFold == 5 e os Plots das redes neurais com neuralnet
